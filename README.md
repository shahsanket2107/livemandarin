...
| `segmenter.min_silence_ms` | 700 | Pause that ends a sentence — lower = faster captions but fragments (measured on a real call: 400 ms mangled names and terms) |
| `segmenter.soft_max_s` / `hard_max_s` | 8 / 12 | Long stretches are cut at a short pause after 8 s, unconditionally at 12 s |
| `asr.carry_context` | true | Feed the previous sentence to the recognizer so names and terms stay consistent |
